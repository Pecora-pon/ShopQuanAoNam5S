            ...
            <div class="row d-flex justify-content-center">
                <div class="col-lg-8">
                    <div class="product__details__text">
                        <form:form action="/them-gio-hang/${sp.tenSanPham}/${sp.hinhAnhURL}" modelAttribute="sp"
                                   method="post">
                            <div class="product-details">
                                <h4>${sp.tenSanPham}</h4>
                                <div class="price">
                                    <h3><fmt:formatNumber value="${sp.giaSanPham}" pattern="#,##0"/></h3>
                                </div>

                                <!-- Hidden input fields -->
                                <input type="hidden" id="tenSanPham" value="${sp.tenSanPham}"/>
                                <input type="hidden" id="hinhAnhURL" value="${sp.hinhAnhURL}"/>
                                <input type="hidden" id="sizeID" value="${sp.size.sizeID}"/>
                                <input type="hidden" id="mauSacID" value="${sp.mauSac.mauSacID}"/>
                                ...
                                <div class="product__details__cart__option">
                                    ...
                                    <div class="buttons">
                                        <button class="primary-btn" onclick="validateAndSubmit(event)">Thêm vào giỏ hàng</button>
                        ...
                        <script>
                            function updateAvailableQuantity() {
                                ...
                                var hinhAnhURL = document.getElementById('hinhAnhURL').value;

                                // Construct the fetch URL with correct parameters
                                $.ajax({
                            ...
                            // Attach the event listeners for size and color changes
                            document.querySelectorAll('input[name="sizeID"]').forEach(radio => radio.addEventListener('change', updateAvailableQuantity));
                            document.querySelectorAll('input[name="mauSacID"]').forEach(radio => radio.addEventListener('change', updateAvailableQuantity));

                            // Call the function initially to set the initial available quantity
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="tabs-5" role="tabpanel">
                                <div class="product__details__tab__content">
                                    <p class="note">${sp.tenSanPham} là một trong những item nổi bật
                                        của ${sp.thuongHieu.tenThuongHieu}, với thiết kế đơn giản:</p>
                                    <div class="product__details__tab__content__item">
                                        <h5>Thông tin sản phẩm</h5>
                                        <p>${sp.tenSanPham} không chỉ mang lại cảm giác thoải mái mà còn có thiết kế hỗ
                                            trợ cho nhiều hoạt động khác nhau. Đường may chất lượng cao và việc chọn
                                            kích thước phù hợp giúp người mặc tự tin và thoải mái khi tham gia các hoạt
                                            động vận động.

                                            Trong khi giữ vững chất lượng, giá cả của chiếc áo này cũng phản ánh giá trị
                                            đồng giá, tạo nên sự cân bằng tốt giữa chất lượng và chi phí.</p>
                                        <p></p>
                                    </div>
                                    <div class="product__details__tab__content__item">
                                        <h5>Chất lượng sản phẩm</h5>
                                        <p>Được may từ chất vải sợi S.Cafe bảo vệ môi trường, thân thiện với làn da
                                            người mặc
                                            Vải cafe mang sự mát mẻ,nhanh khô, khử mùi tốt và chống được tia cực tím,
                                            tia UV
                                            Form áo rộng rãi, phù hợp với mọi vóc dáng mang lại sự thoải mái cho người
                                            mặc
                                        ...
                                        <div class="customer-reviews">
                                            <c:forEach var="item" items="${rv}">
                                                <div class="customer-review"
                                                     style="margin-bottom: 15px; border-bottom: 1px solid #ccc; padding-bottom: 15px;">
                                                    <span class="customer-name"
                                                          style="font-size: 19px; font-weight: bold; color: #333; font-style: italic;"><c:out
                                                            value="${item.khachHang.hoTen} :"/></span>
                                                    <div class="product__cart__item__pic">
                                                        <img src="/getimage/${item.hinhAnhURL}"
                                                    ...
                                                    <div class="product__cart__item__text">
                                                        ...
                                                        <h7>Kích thước: <c:out
                                                                value="${item.sanPham.size.tenSize} "/></h7>
                                                        <h7>Màu sắc: <c:out
                                                                value="${item.sanPham.mauSac.tenMauSac} "/></h7>
                                                        <br>
                                                        <span class="customer-comment"
                                                              style="font-size: 16px; font-weight: bold; color: #555;"><c:out
                                                                value="${item.binhLuan}"/></span>
                                                    </div>
